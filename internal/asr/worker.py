...
def main() -> int:
    ...
    for segment in segments:
        ...
        if not text:
            continue
        start = max(args.offset, args.offset + float(segment.start))
        end = max(start + 0.05, args.offset + float(segment.end))
        words = []
        for word in segment.words or []:
            ...
            if not value or word.start is None or word.end is None:
                continue
            words.append({
                "start": args.offset + float(word.start),
                "end": args.offset + float(word.end),
                "text": value,
