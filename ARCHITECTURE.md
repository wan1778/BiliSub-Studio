...
Voice/TTS uses the verified NGHI Ngọc Huyền ONNX/config pair through pinned local Piper. One model is loaded per job and reused; each uncached subtitle cue is synthesized once as a complete text, never split by Whisper pauses. Project cues fit their mapped Whisper speech envelope within the unchanged SRT cue. Chained atempo and measured frame-count correction produce the exact target length, with only a tiny padding remainder and no tail truncation; failure to fit prevents master promotion. Strong tempo changes require listening review even when duration matches. The text-only sample explicitly keeps natural duration when it fits its window, without fabricated Whisper data. Cache keys bind the timing-policy revision, speech envelope, model/config hashes, runtime/worker identity, cue time and text. C# verifies each clip's SHA and actual PCM frame count as well as result order, master hash and decoded format/duration. A bounded PCM-to-FLAC pipeline creates a unique master per run; cancellation preserves previous outputs. See docs/engineering/EDITOR_NGHITTS_AUDIT.md for artifacts/history and docs/engineering/VOICE_SOURCE_DURATION.md for current fitting policy and NOT RUN validation status.
