...
Accurate OCR uses adaptive frame timing. FFmpeg preserves every decoded source-frame PTS, while PaddleOCR normally evaluates 4 samples per second. A blank/text or changed-text sample transition opens the buffered native-frame interval between those samples. When the sampled text is stable, the same long-lived worker runs a lightweight OpenCV concentrated-edge comparison over every consecutive native frame; only a visual transition and its two following frames are nominated for PaddleOCR. Core submits nominated frames through the validated four-image batch protocol and feeds results to the exact-frame tracker in PTS order. Thus every frame is covered by either direct Paddle refinement or the lightweight probe, while stable intervals avoid full-frame Paddle inference. Per-lane telemetry is capped at two updates per second, and WinUI accumulates live cues by millisecond start key while rebuilding the visible list at most every two seconds.
...
`LocalAsrInstaller.Gpu.cs` owns on-demand Windows x64 GPU dependencies under `Tools/ASR/gpu`, independently of OCR and machine-wide CUDA/PATH. The automatic ASR GPU path first requires NVIDIA with a driver reporting CUDA >=12.8 and sufficient free VRAM when telemetry is available. It downloads pinned NVIDIA cuBLAS 12.8.4.1, CUDA runtime 12.8.90, NVRTC 12.8.93 and cuDNN 9.10.2.21 wheels (about 1.3 GB once), verifies exact archive size/SHA-256, and extracts only DLLs plus NVIDIA licenses into unique generations. Atomic manifests bind package identity and extracted file hashes; reuse rechecks DLL hashes, incomplete installs rebuild from verified archives, and canceled transfers retain resumable partials without publishing readiness. Old generations are not overwritten while workers may hold DLLs. The worker receives private `--cuda-bin` paths for both probe and transcription, registers process-local DLL directories before importing Whisper, preloads cuBLAS/cuDNN, and retains loader handles. CPU workers do not require these directories. No driver, global PATH or CUDA Toolkit installation is performed. Dependency readiness is not GPU runtime PASS: the existing real-audio probe remains the final selector, with explicit CPU fallback on preparation or inference failure. See `docs/engineering/ASR_GPU_AUTO_SETUP.md` for artifact provenance and validation status.

The Voice inspector also exposes explicit CPU and optional Hybrid ASR modes; GPU-preferred remains the default. Hybrid owns two reused Whisper models in one cancelable Python process, dynamically assigning bounded overlapping chunks to CPU/GPU threads. Word seams are reconciled before ordered chunk-level checkpoint commits; Hybrid uses a separate checkpoint key/file and resumes only from its durable frontier. Hardware admission and a real-audio two-device probe precede selection, with logged GPU/CPU fallback on probe failure. Manual reanalysis reuses valid checkpoints, preserves the old analysis file through handoff and updates dependent voice only after success; it never rewrites imported Vietnamese SRT. See `docs/engineering/ASR_HYBRID.md` for concurrency, timing limitations and NOT RUN field-validation status.

ASR runtime manifests use one shared snake_case JSON policy for both writing and readiness checks. Offline contracts exercise the actual installer write/read round-trip, reopen, corruption and version/worker mismatch rejection. ASR checkpoint publication uses `AsrCheckpointFile`: closed/flushed same-directory snapshots, atomic replacement with a previous-file backup, bounded Windows sharing/access retries and explicit path/HRESULT diagnostics. Complete failed publications are retained for recovery; they are not auto-trusted. Loading prefers the primary, then a source/model-validated backup, with delete-sharing readers. No permissions are changed. See docs/engineering/ASR_CHECKPOINT_REPLACE.md for the NOT RUN field-validation boundary. The opt-in `--asr-voice-runtime <isolated-root> <short-real-video> <vietnamese-srt>` contract exercises public ASR and TTS jobs end to end with installed real models, verifies runtime reuse, whole-cue cache and processed preview, and never substitutes fake speech provenance.
...
Voice/TTS uses the verified NGHI Ngọc Huyền ONNX/config pair through pinned local Piper. One model is loaded per job and reused; each attempt synthesizes a complete cue, never split by Whisper pauses. Project cues target their mapped Whisper speech envelope within the unchanged SRT cue using model-native `length_scale`, with at most ten whole-cue attempts and bounded relative scales. There is no post-synthesis playback-speed filter. The accepted model output is preserved intact, with trailing padding limited to 40 ms and 2% of the target; failure to fit prevents master promotion. Strong native-rate changes require listening review even when duration matches. The text-only sample explicitly keeps natural duration when it fits its window, without fabricated Whisper data. Cache keys bind the native timing-policy revision, speech envelope, model/config hashes, runtime/worker identity, cue time and text. C# verifies native synthesis metadata, padding/call counts, each clip's SHA and actual PCM frame count as well as result order, master hash and decoded format/duration. Progress shows the current real attempt without inflating cue-completion percentage. A bounded PCM-to-FLAC pipeline creates a unique master per run; cancellation preserves previous outputs. See docs/engineering/EDITOR_NGHITTS_AUDIT.md for artifacts/history and docs/engineering/VOICE_SOURCE_DURATION.md for current fitting policy and NOT RUN validation status.
