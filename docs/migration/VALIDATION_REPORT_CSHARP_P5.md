...
- The replacement source merges `XamlControlsResources`, records every startup phase under `%LOCALAPPDATA%\BiliSub Studio\Logs`, shows fatal startup errors, runs the exact published EXE through a CI launch sentinel, locks Setup to the per-user program directory and limits satellite resources to Vietnamese/English.
