...
# IMAGE-05 — .jpeg must work through the complete Image/Logo path:
...
require('private static readonly string[] ImageExtensions = [".png", ".jpg", ".jpeg"];' in picker,
        "IMAGE-05 Windows picker must include .jpeg")
...
require("ImageExtensions" in pick_image and "*.png;*.jpg;*.jpeg" in pick_image,
        "IMAGE-05 both WinRT and Win32 image pickers must expose JPEG")
...
require("extensions.Contains(extension, StringComparer.OrdinalIgnoreCase)" in validate_pick,
        "IMAGE-05 picked JPEG extension must be validated case-insensitively")
...
require(add.count("await _picker.PickImageAsync();") == 1,
        "IMAGE-05 Add Image must use the image picker exactly once")
require('extension is not (".png" or ".jpg" or ".jpeg")' in add,
        "IMAGE-05 Add Image source validation must accept .jpeg")
require("BitmapDecoder.CreateAsync(stream)" in add,
        "IMAGE-05 JPEG dimensions must come from the Windows bitmap decoder")
require("decoder.PixelWidth" in add and "decoder.PixelHeight" in add,
        "IMAGE-05 JPEG source dimensions must be retained")
require("Path.GetFullPath(path)" in add,
        "IMAGE-05 JPEG overlay state must keep a canonical source path")
...
require("StorageFile.GetFileFromPathAsync(path)" in load_bitmap,
        "IMAGE-05 Preview must reopen the JPEG source file")
require("var bitmap = new BitmapImage();" in load_bitmap
        and "await bitmap.SetSourceAsync(stream);" in load_bitmap,
        "IMAGE-05 Preview must decode JPEG through BitmapImage")
...
require("Source = bitmap" in render,
        "IMAGE-05 Preview must render the decoded JPEG")
require("Opacity = state.Opacity" in render,
        "IMAGE-05 Preview must apply editor opacity to JPEG")
...
require('extension is not (".png" or ".jpg" or ".jpeg")' in normalize_state,
        "IMAGE-05 reopened image state must still accept .jpeg")
require("!File.Exists(path)" in normalize_state,
        "IMAGE-05 reopened JPEG must still exist before restore")
...
require("JsonSerializer.DeserializeAsync<List<EditorImageOverlayState>>" in load_project,
        "IMAGE-05 JPEG placement must reload from the image sidecar")
require("TryNormalizeImageState(image, out var normalized)" in load_project,
        "IMAGE-05 reopened JPEG state must be normalized")
require("await EnsureBitmapLoadedAsync(_imageOverlays[index].Path);" in load_project,
        "IMAGE-05 reopened JPEG must be decoded for Preview again")
...
require('extension is not (".png" or ".jpg" or ".jpeg")' in normalize_export,
        "IMAGE-05 Export validation must accept .jpeg")
require("!File.Exists(path)" in normalize_export,
        "IMAGE-05 Export must reject a missing JPEG source")
...
require('args.AddRange(["-loop", "1", "-framerate", "1", "-i", image.Path]);' in render_export,
        "IMAGE-05 JPEG must be supplied to FFmpeg as a persistent still-image input")
...
require("format=rgba" in graph,
        "IMAGE-05 Export must promote opaque JPEG pixels to RGBA before opacity is applied")
require("colorchannelmixer=aa={alpha}" in graph,
        "IMAGE-05 Export must implement editor opacity for JPEG through the alpha channel")
require("overlay={x}:{y}" in graph,
        "IMAGE-05 JPEG must be composited with the FFmpeg overlay filter")

# JPEG has no source alpha. Preview treats it as opaque and applies Image.Opacity.
...
for opacity in (1.0, 0.80, 0.50, 0.20, 0.05):
    require(abs(preview_alpha(opacity) - export_alpha(opacity)) < 1e-12,
            f"IMAGE-05 fixture: JPEG opacity parity failed at {opacity}")

print("PASS: IMAGE-05 JPEG import, Preview, reopen and Export contract")
