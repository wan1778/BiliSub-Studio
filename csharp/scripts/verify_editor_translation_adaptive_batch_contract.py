...
require(MEMORY.is_file(), f"LIVE-SRT-01 missing {MEMORY.relative_to(ROOT)}")
...
for token in (
    ...
    require(token in source, f"LIVE-SRT-01 direct cue + memory contract missing: {token}")

for token in (
    "private const string MemoryTranslationSchema",
    "BuildTranslationPromptMemory",
    "ValidateMemoryBatch",
    "checkpoint.Names",
    "checkpoint.Relations",
):
    require(token in memory, f"LIVE-SRT-01 locked-memory helper missing: {token}")
...
require(save_pos < offset_pos < live_pos,
        "LIVE-SRT-01 must persist cue + learned memory before publishing progress that wakes the Editor checkpoint reader")
...
print("PASS: LIVE-SRT-01 keeps one-cue realtime translation while persisting locked glossary/name/relation memory before progress publication")
