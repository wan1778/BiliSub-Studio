...
for token in (
    ...
    'ModelMode: modelMode',
    'modeScope = modelMode == EditorTranslationModelMode.Fast ? "fast" : string.Empty',
):
...
# Synthetic identity/default + checkpoint-path compatibility contract.
...
def scope_for(base: str, selected: str) -> str:
    return base + ('fast' if selected == fast else '')
require(scope_for('all', quality) == 'all', "SPEED-05 quality mode must keep the legacy all-checkpoint path")
require(scope_for('cue', quality) == 'cue', "SPEED-05 quality mode must keep the legacy cue-checkpoint path")
require(scope_for('all', fast) != 'all', "SPEED-05 fast mode must isolate all-checkpoint path from legacy quality")
require(scope_for('cue', fast) != 'cue', "SPEED-05 fast mode must isolate cue-checkpoint path from legacy quality")

print("PASS: SPEED-05 keeps 8B quality as default, preserves legacy 8B checkpoint paths, and exposes explicit isolated 4B Fast/Draft mode")
