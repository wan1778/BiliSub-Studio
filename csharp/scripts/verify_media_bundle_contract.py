...
require("int EndpointOffset = 0" in models, "endpoint rotation must be job-local through VideoResolveRequest")
...
require("Math.Abs(request.EndpointOffset) % endpoints.Count" in resolver, "resolver must rotate endpoint selection deterministically per job refresh")
require("endpointOffset++" in download and "resolveRequest with { EndpointOffset = endpointOffset }" in download, "Range refresh must advance to the next playurl endpoint instead of resolving index zero again")
require("CDN {kind}: chuyển" in download and "giữ nguyên Range resume" in download, "CDN rotation must be visible in the shared diagnostic log")
require("playurl refresh vẫn trả cùng tuyến" in download, "same-route refresh must be detected rather than reported as successful failover")
require("yt-dlp chỉ còn là fallback cuối" in download, "yt-dlp must remain the last resort after real CDN rotation")
...
print("PASS: root-cause CDN failover + long-media/highest-quality/default-all/separate-assets/short-read/HTTP503/adaptive-range/thumbnail/optional-subtitle/resume contracts")
