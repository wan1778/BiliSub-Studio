...
require("IReadOnlyList<string>? EndpointUrls = null" in models, "resolved streams must retain primary + backup CDN endpoints")
require("int EndpointIndex = 0" in models, "resolved streams must expose the active CDN endpoint index")
require("int EndpointOffset = 0" in models, "endpoint selection must remain explicit in VideoResolveRequest")
require("EndpointDiscoveryWarning" in models, "endpoint discovery failures must remain diagnosable")
require("info.Thumbnail" in resolver and '[JsonPropertyName("thumbnail")]' in resolver, "resolver must expose yt-dlp thumbnail")

# Root-cause contract: yt-dlp selects the format, while raw Bilibili playurl owns CDN redundancy.
require("x/player/wbi/playurl" in playurl and "w_rid" in playurl and "wts" in playurl, "raw endpoint discovery must use signed Bilibili WBI playurl")
require("x/player/pagelist" in playurl and "ResolveCidAsync" in playurl, "raw endpoint discovery must resolve the exact BV part/cid")
require('"baseUrl", "base_url", "url"' in playurl, "playurl client must retain the primary endpoint")
...
require("currentStreams[StreamKind.Video] = selection.Video" in download and "currentStreams[StreamKind.Audio] = selection.Audio" in download, "video and audio must own independent current CDN routes")
require("EndpointUrls(existing)" in download and "nextIndex = existing.EndpointIndex + 1" in download, "recovery must advance through the current endpoint set first")
require("Url = endpoints[nextIndex]" in download and "EndpointIndex = nextIndex" in download, "cached backup rotation must actually switch the active URL/index")
require("dùng backup đã có, giữ nguyên Range resume" in download, "cached CDN rotation must be visible in the shared diagnostic log")
require("đã thử hết" in download and "refetch playurl vòng" in download, "playurl refetch must occur only after the current endpoint set is exhausted")
require("refreshRounds = new Dictionary<StreamKind, int>()" in download, "playurl refetch rounds must be tracked independently per stream")
require("resolveRequest with { EndpointOffset = 0 }" in download, "a fresh playurl candidate set must start from its primary endpoint")
require("fresh.FormatId" in download and "existing.FormatId" in download and "dừng để tránh trộn dữ liệu khác stream" in download, "playurl refresh must reject format identity changes before resuming partial bytes")
require("existing.Size > 0" in download and "fresh.Size > 0" in download and "dừng để tránh hỏng file resume" in download, "playurl refresh must reject known stream-size changes before resuming partial bytes")
require("playurl refresh vẫn trả cùng tuyến" in download, "same-route refetch must be detected rather than reported as meaningful failover")
require("currentStreams.TryGetValue(stream.Kind" in download, "adaptive one-connection retry must use the latest route for the same stream only")
require("yt-dlp chỉ còn là fallback cuối" in download, "yt-dlp must remain the last resort after CDN rotation/refetch/adaptive Range")
require("token=primary-secret" not in download and "token=backup" not in download, "production diagnostic source must never contain fixture signed-token values")

# Deterministic executable gates must prove discovery and exact-offset failover behavior.
require("new Uri(value).Host" in cdn_fixture and "primary + backup CDN endpoints" in cdn_fixture, "CI must execute a behavioral raw-playurl primary/backup fixture")
require("const int TinyRead = 379" in cdn_failover_fixture, "failover fixture must preserve the real 379-byte regression")
require("backup-cdn.fixture" in cdn_failover_fixture, "failover fixture must use a genuinely different backup host")
require("third.Start != TinyRead * 2L" in cdn_failover_fixture, "failover fixture must assert the exact missing byte after two primary short reads")
require("SequenceEqual(actual)" in cdn_failover_fixture, "failover fixture must verify byte-for-byte final output")
require("primary CDN short-read rotated to backup CDN and preserved the exact Range offset" in cdn_failover_fixture, "failover fixture PASS marker is missing")
require("BiliSubStudio.CdnRegression" in workflow, "workflow must run raw playurl endpoint discovery")
require("BiliSubStudio.CdnFailoverRegression" in workflow, "workflow must run the primary-to-backup exact-offset regression")
...
require("weakProgressFailures % 2 == 0" in range_downloader, "repeated tiny reads must request transport refresh/rotation")
require("immediateRefresh = error is HttpRequestException" in range_downloader, "explicit HTTP transport failures must request endpoint refresh immediately")
...
require("GetBaseException().Message" in download, "final Range failure log must expose the root transport cause")
